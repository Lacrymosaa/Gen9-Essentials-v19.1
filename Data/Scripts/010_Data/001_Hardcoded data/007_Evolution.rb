...
GameData::Evolution.register({
  ...
  :use_item_proc => proc { |pkmn, item, parameter|
    ...
    case pkmn.species
    when :QUILAVA
      next false if pkmn.level < 36
      pkmn.form = 1 if item == :FIRESTONE
      next true
    when :DEWOTT
      next false if pkmn.level < 36
      pkmn.form = 1 if item == :WATERSTONE
      next true
    when :DARTRIX
      next false if pkmn.level < 34
      pkmn.form = 1 if item == :LEAFSTONE
      next true
    when :URSARING
      if item == :MOONSTONE
        pkmn.form = 0 
      elsif item == :DUSKSTONE
        pkmn.form = 1  
      else
        next false
      end
    ...
    when :PETILIL
      ...
      next true
    when :RUFFLET
      next false if pkmn.level < 54
      pkmn.form = 1 if item == :DAWNSTONE
      next true
    when :GOOMY
      next false if pkmn.level < 40
      pkmn.form = 1 if item == :WATERSTONE
      next true
    when :BERGMITE
      next false if pkmn.level < 37
      pkmn.form = 1 if item == :ICESTONE
      next true
    else
      next false
    end
...
GameData::Evolution.register({
  ...
  :use_item_proc => proc { |pkmn, item, parameter|
    next false unless item == parameter
    if item == :SCROLLOFWATERS
      pkmn.form = 1
      next true
    elsif item == :SCROLLOFDARKNESS
      pkmn.form = 0
      next true
    else
      next false 
...
GameData::Evolution.register({
  ...
  :parameter     => Integer,
  :level_up_proc => proc { |pkmn, parameter|
    if pkmn.level >= parameter && $game_switches[1150] 
       next true
    end
    next false
  }
...
GameData::Evolution.register({
  ...
  :parameter     => Integer,
  :level_up_proc => proc { |pkmn, parameter|
    if pkmn.level >= parameter && $game_switches[451]
       next true
    end
    next false
  }
...
GameData::Evolution.register({
  ...
  :parameter     => Integer,
  :level_up_proc => proc { |pkmn, parameter|
    if pkmn.level >= parameter && $game_switches[634] 
       next true
    end
    next false
  }
...
GameData::Evolution.register({
  ...
  :parameter         => Integer,
  :on_battle_proc => proc { |pkmn, parameter|
    next pkmn.level >= parameter && $game_switches[418]
  }
})
...
GameData::Evolution.register({
  ...
  :parameter     => :Item,
  :use_item_proc => proc { |pkmn, parameter, item|
    next item == parameter && pkmn.level >= 35
  }
})
