...
BattleHandlers::HPHealItem.add(:GARLIC,
  proc { |item,battler,battle,forced|
    works = pbBattleConfusionBerry(battler,battle,item,forced,3, _INTL("For {1}, the {2} was too dry!",battler.pbThis(true),GameData::Item.get(item).name))

    if works
      battle.battlers[1].pbConfuse if battle.battlers[1].pbCanConfuseSelf?(false)
    end
...
BattleHandlers::HPHealItem.add(:CARROT,
  proc { |item,battler,battle,forced|
    next false if !battler.canHeal?
    next false if !forced && !battler.canConsumePinchBerry?(false)
    amt = battler.totalhp / 4
    ...
    battler.pbRaiseStatStage(:ACCURACY,(Settings::X_STAT_ITEMS_RAISE_BY_TWO_STAGES) ? 4 : 2,battler)

    next true
  }
)

BattleHandlers::HPHealItem.add(:PUMPKIN,
  proc { |item,battler,battle,forced|
    next false if !battler.canHeal?
    next false if !forced && !battler.canConsumePinchBerry?(false)
...
BattleHandlers::HPHealItem.add(:FRUIT,
  proc { |item,battler,battle,forced|
    next false if !battler.canHeal?
    next false if !forced && !battler.canConsumePinchBerry?(false)
    amt = battler.totalhp
    ...
    battle.pbDisplay(_INTL("{1} became healthy.",battler.pokemon.name))
...
BattleHandlers::DamageCalcUserItem.add(:NUNCHAKUS,
  proc { |item,user,target,move,mults,baseDmg,type|
    if (type == :FIGHTING || type == :STEEL)
