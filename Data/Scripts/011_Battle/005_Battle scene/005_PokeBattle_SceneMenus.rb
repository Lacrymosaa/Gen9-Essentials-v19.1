...
class BattleMenuBase
  ...
  TEXT_BASE_COLOR_WHITE   = PokeBattle_SceneConstants::MESSAGE_BASE_COLOR_WHITE
  TEXT_SHADOW_COLOR_NEGRO = PokeBattle_SceneConstants::MESSAGE_SHADOW_COLOR_NEGRO
...
class CommandMenuDisplay < BattleMenuBase
  ...
  def initialize(viewport,z)
    ...
    @msgBox = Window_UnformattedTextPokemon.newWithSize("",
       self.x+30,self.y+2,220,Graphics.height-self.y,viewport)
    @msgBox.baseColor   = TEXT_BASE_COLOR_WHITE
    @msgBox.shadowColor = TEXT_SHADOW_COLOR_NEGRO
...
class FightMenuDisplay < BattleMenuBase
  ...
  PP_COLORS = [
     ...
     TEXT_BASE_COLOR_WHITE,TEXT_SHADOW_COLOR_NEGRO           # Black, more than 1/2 of total PP
  ]
...
class TargetMenuDisplay < BattleMenuBase
  ...
  def refreshButtons
    ...
    @buttons.each_with_index do |button,i|
      ...
      x = button.x-self.x+button.src_rect.width/2
      y = button.y-self.y+2
      textpos.push([@texts[i],x,y,2,TEXT_BASE_COLOR_WHITE,TEXT_SHADOW_COLOR])
    end
    pbDrawTextPositions(@overlay.bitmap,textpos)
  end
