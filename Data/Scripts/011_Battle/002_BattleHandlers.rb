...
def pbBattleConfusionBerry(battler,battle,item,forced,flavor,confuseMsg)
  ...
  if amt>0
    if forced
      ...
      battle.pbDisplay(_INTL("Los PS de {1} se han recuperado.",battler.pbThis))
    else
      battle.pbDisplay(_INTL("{1} recuperó su vida gracias a {2}!",battler.pbThis,itemName))
    end
  end
...
def pbBattleMoveImmunityHealAbility(user,target,move,moveType,immuneType,battle)
  ...
  if target.canHeal? && target.pbRecoverHP(target.totalhp/4)>0
    if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("Los PS de {1} se han recuperado.",target.pbThis))
    else
      battle.pbDisplay(_INTL("{2} recuperço los PS de {1}.",target.pbThis,target.abilityName))
    end
  else
    if PokeBattle_SceneConstants::USE_ABILITY_SPLASH
      battle.pbDisplay(_INTL("It doesn't affect {1}...",target.pbThis(true)))
...
def pbBattleWeatherAbility(weather,battler,battle,ignorePrimal=false)
  ...
  if !PokeBattle_SceneConstants::USE_ABILITY_SPLASH
    battle.pbDisplay(_INTL("¡Se ha activado {2} de {1}!",battler.pbThis,battler.abilityName))
  end
  fixedDuration = false
