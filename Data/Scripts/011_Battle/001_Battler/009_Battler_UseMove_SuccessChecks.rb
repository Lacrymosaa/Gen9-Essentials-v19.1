class PokeBattle_Battler
  ...
  def pbCanChooseMove?(move,commandPhase,showMessages=true,specialUsage=false)
    ...
    # Blood Moon
    if move.id == :BLOODMOON && @effects[PBEffects::BloodMoon] > 0
      if showMessages
        msg = _INTL("{1} can't use {2} again right away!",pbThis,move.name)
        (commandPhase) ? @battle.pbDisplayPaused(msg) : @battle.pbDisplay(msg)
      end
      return false
    end
  ...
  def pbSuccessCheckAgainstTarget(move,user,target)
    ...
    if move.canProtectAgainst?
      ...
      # Silk Trap
      if target.effects[PBEffects::SilkTrap] && !unseenfist
      ...
      if target.effects[PBEffects::BeakBlast]
        if move.pbContactMove?(user) && user.affectedByContactEffect? && user.pbCanBurn?(target, false, move)
          user.pbBurn(target)
          @battle.pbDisplay(_INTL("{1} was burned by the heated beak!", user.pbThis))
        end
      end
      # Baneful Bunker
      ...
      # Burning Bulwark
      if target.effects[PBEffects::BurningBulwark] && !unseenfist
        @battle.pbCommonAnimation("BurningBulwark", target)
        @battle.pbDisplay(_INTL("{1} protected itself with burning flames!", target.pbThis))
        target.damageState.protected = true
        @battle.successStates[user.index].protected = true
        if move.pbContactMove?(user) && user.affectedByContactEffect?
