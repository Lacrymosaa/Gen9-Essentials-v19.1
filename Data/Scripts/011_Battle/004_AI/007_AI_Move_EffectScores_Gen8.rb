  ...
  def pbGetMoveScoreFunctionCode(score,move,user,target,skill = 100)
    ...
    when "17A"
      good_effects = [:Reflect, :LightScreen, :AuroraVeil, :SeaOfFire,
                      ...
                      :Tailwind].map!{ |e| PBEffects.const_get(e) rescue nil }
      bad_effects = [:Spikes, :StickyWeb, :ToxicSpikes, :StealthRock].map!{|e| PBEffects.const_get(e) rescue nil }
      if skill >= PBTrainerAI.mediumSkill
    ...
    when "188"
      if skill >= PBTrainerAI.highSkill
        ...
        score += 50 if target.stages[stat] > 1
      end
    #---------------------------------------------------------------------------
    when "189"
