class PokeBattle_AI
  ...
  def pbGetMoveScoreFunctionCode(score,move,user,target,skill=100)
    return 0 if target.effects[PBEffects::Commander] &&
            (!target.isCommanderHost? || target.effects[PBEffects::Commander].length == 1)
    case move.function
    #---------------------------------------------------------------------------
      ...
      if skill>=PBTrainerAI.mediumSkill
        if !target.ability || user.ability==target.ability ||
           ...
           [:FLOWERGIFT, :FORECAST, :ILLUSION, :IMPOSTER, :MULTITYPE, :RKSSYSTEM,
            :TRACE, :WONDERGUARD, :ZENMODE, :GULPMISSILE, :ICEFACE, :NEUTRALIZINGGAS, :COMMANDER].include?(target.ability_id)
          score -= 90
        end
      end
      if skill>=PBTrainerAI.highSkill
    ...
    when "067"
      ...
      if skill>=PBTrainerAI.mediumSkill
        if (!user.ability && !target.ability) ||
           ...
           [:ILLUSION, :MULTITYPE, :RKSSYSTEM, :WONDERGUARD, :ICEFACE, :GULPMISSILE, :NEUTRALIZINGGAS, :COMMANDER].include?(user.ability_id) ||
           [:ILLUSION, :MULTITYPE, :RKSSYSTEM, :WONDERGUARD, :ICEFACE, :GULPMISSILE, :NEUTRALIZINGGAS, :COMMANDER].include?(target.ability_id)
          score -= 90
        end
      end
      if skill>=PBTrainerAI.highSkill
