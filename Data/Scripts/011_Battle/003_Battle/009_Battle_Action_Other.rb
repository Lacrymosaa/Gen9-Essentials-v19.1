class PokeBattle_Battle
  ...
  def pbHasMegaRing?(idxBattler)
    ...
    Settings::MEGA_RINGS.each { |item| return true if $PokemonBag.pbHasItem?(item) && !$PokemonBag.pbHasItem?(:GOLDENMASKON)}
    return false
  end
...
#    end
  ...
  def pbCanMegaEvolve?(idxBattler)
    ...
    echo "Cosas para poder megaevolucionar"

    if @battlers[idxBattler] == 0 && @battlers[idxBattler].pokemon.species == :MIGHTYENA
      return false if !PBDayNight.isNight?
    end
  ...
  def pbMegaEvolve(idxBattler)
    ...
    when 1   # Rayquaza
      ...
      pbDisplay(_INTL("{1}'s {2} está reaccionando al {4}'s {3}!",
    ...
    pbDisplay(_INTL("{1} ha Mega Evolucionado en {2}!",battler.pbThis,megaName))
