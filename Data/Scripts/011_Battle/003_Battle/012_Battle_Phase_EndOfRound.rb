class PokeBattle_Battle
  ...
  def pbEORWeather(priority)
    ...
    if @field.weatherDuration==0
      case @field.weather
      when :Sun       then pbDisplay(_INTL("El sol se fue."))
      when :Rain      then pbDisplay(_INTL("La lluvia paró."))
      when :Sandstorm then pbDisplay(_INTL("La tormenta de arena amainó."))
      when :Hail      then pbDisplay(_INTL("El nieve paró."))
      when :ShadowSky then pbDisplay(_INTL("The shadow sky faded."))
      when :Fog       then pbDisplay(_INTL("La niebla desapareció."))
      end
      @field.weather = :None
    ...
    case @field.weather
#    when :Sun         then pbDisplay(_INTL("El sol está durito."))
#    when :Rain        then pbDisplay(_INTL("La lluvia sigue cayendo."))
    when :Sandstorm   then pbDisplay(_INTL("La tormenta de arena sigue presente."))
    when :Hail        then pbDisplay(_INTL("El nieve sigue cayendo."))
#    when :HarshSun    then pbDisplay(_INTL("El sol quema demasiado, parece Almería."))
#    when :HeavyRain   then pbDisplay(_INTL("Está lloviendo muy fuerte."))
#    when :StrongWinds then pbDisplay(_INTL("El viento es fuerte."))
  ...
  def pbEndOfRoundPhase
    ...
    # Salt (Merequetengue)
    priority.each do |b|
      next if !b.effects[PBEffects::Salted] || !b.takesIndirectDamage?
      oldHP = b.hp

      # Damage dealt depends on pkmn type (Steel or Water)
      if b.takesMoreSaltDamage?
        b.pbReduceHP(b.totalhp/4)
      else
        b.pbReduceHP(b.totalhp/8)
      end

      pbDisplay(_INTL("{1} sufre de los efectos del curado!",b.pbThis))
      b.pbItemHPHealCheck
      b.pbAbilitiesOnDamageTaken(oldHP)
      b.pbFaint if b.fainted?
    end
    ...
    # Glaive Rush
    pbEORCountDownBattlerEffect(priority,PBEffects::GlaiveRush) { |battler|
      pbDisplay(_INTL("{1} is no longer vulnerable!",battler.pbThis))
    }
    # Blood Moon
    pbEORCountDownBattlerEffect(priority,PBEffects::BloodMoon) { |battler|
      pbDisplay(_INTL("{1} is able to use Blood Moon again!",battler.pbThis))
    }
    @battlers.each do |b|
      next if b.fainted?
      if b.effects[PBEffects::SyrupBomb] > 0
        if b.pbCanLowerStatStage?(:SPEED, b)
          b.pbLowerStatStage(:SPEED, 1, b)
        end
        b.effects[PBEffects::SyrupBomb] -= 1
        if b.effects[PBEffects::SyrupBomb] == 0
          @scene.pbDisplay(_INTL("{1} is no longer slowed by syrup!", b.pbThis))
        end
      end
    end
    # Heal Block
    ...
    eachBattler do |b|
      ...
      b.effects[PBEffects::BurningBulwark]    = false
