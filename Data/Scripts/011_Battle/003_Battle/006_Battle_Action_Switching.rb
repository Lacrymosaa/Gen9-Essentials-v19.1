class PokeBattle_Battle
  ...
  def pbCanSwitchLax?(idxBattler,idxParty,partyScene=nil)
    ...
    if party[idxParty].egg?
      partyScene.pbDisplay(_INTL("¡Un huevo no puede combatir, loco!")) if partyScene
      return false
    end
    if !pbIsOwner?(idxBattler,idxParty)
      owner = pbGetOwnerFromPartyIndex(idxBattler,idxParty)
      partyScene.pbDisplay(_INTL("¡No puedes cambiar el Pokemon de {1} por uno de los tuyoss!",
        owner.name)) if partyScene
      return false
    end
    if party[idxParty].fainted?
      partyScene.pbDisplay(_INTL("¡{1} no tiene suficiente energía para combatir!",
         party[idxParty].name)) if partyScene
      return false
    end
    if pbFindBattler(idxParty,idxBattler)
      partyScene.pbDisplay(_INTL("¡{1} ya está en combate!",
         party[idxParty].name)) if partyScene
      return false
    end
    return true
  ...
  def pbCanSwitch?(idxBattler,idxParty=-1,partyScene=nil)
    ...
    if battler.effects[PBEffects::Commander]
    ...
    if battler.trappedInBattle?
      partyScene.pbDisplay(_INTL("¡{1} no se puede cambiar!",battler.pbThis)) if partyScene
      return false
    end
    ...
    eachOtherSideBattler(idxBattler) do |b|
      ...
      if BattleHandlers.triggerTrappingTargetAbility(b.ability,battler,b,self)
        partyScene.pbDisplay(_INTL("¡El {1} de {2} previene un cambio!",
    ...
    loop do
      ...
      @battlers.each do |b|
        ...
        if !pbOwnedByPlayer?(idxBattler)   # Opponent/ally is switching in
          ...
          switch = false
          if !pbDisplayConfirm(_INTL("¿Quieres usar otro Pokémon?"))
  ...
  def pbMessageOnRecall(battler)
    if battler.pbOwnedByPlayer?
      if battler.hp<=battler.totalhp/4
        pbDisplayBrief(_INTL("¡Buen trabajo, {1}! ¡Vuelve aquí!",battler.name))
      elsif battler.hp<=battler.totalhp/2
        pbDisplayBrief(_INTL("¡Es suficiente, {1}! ¡Vuelve aquí!",battler.name))
      elsif battler.turnCount>=5
        pbDisplayBrief(_INTL("¡{1}, es suficiente! ¡Vuelve aquí!",battler.name))
      elsif battler.turnCount>=2
        pbDisplayBrief(_INTL("¡{1}, vuelve!",battler.name))
      else
        pbDisplayBrief(_INTL("¡{1}, toca cambiar! ¡Vuelve aquí!",battler.name))
      end
    else
  ...
  def pbMessagesOnReplace(idxBattler,idxParty)
    ...
    if pbOwnedByPlayer?(idxBattler)
      ...
      if opposing.fainted? || opposing.hp==opposing.totalhp
        ...
        pbDisplayBrief(_INTL("¡Ve a por ello, {1}!",newPkmnName))
      elsif opposing.hp>=opposing.totalhp/4
        pbDisplayBrief(_INTL("¡Solo un poco más! ¡Adelante, {1}!",newPkmnName))
      else
        pbDisplayBrief(_INTL("¡Tu oponente está débil! ¡Acaba con él, {1}!",newPkmnName))
      end
    else
  ...
  def pbOnActiveOne(battler)
    ...
    if battler.opposes? && battler.shadowPokemon?
      ...
      pbDisplayPaused(_INTL("¡Un Pokémon <c3=FF7373,A04040>oscuro<c3=FFFFFF,000000> ha entrado en combate!"))
      pbWait(3)
      pbDisplayPaused(_INTL("Su <c3=FF7373,A04040>alma<c3=FFFFFF,000000> grita por tu ayuda desde dentro y su furia se acentúa..."))
      battler.pbRaiseStatStage(:ATTACK ,1, battler)
      battler.pbRaiseStatStage(:SPECIAL_ATTACK ,1, battler)
    end
    # Record money-doubling effect of Amulet Coin/Luck IncenseS
